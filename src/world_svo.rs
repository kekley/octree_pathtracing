...
impl Octree<Octree<u32>> {
    pub fn minecraft_world(at: WorldCoords, world: World) -> Self {
        let x_offset = at.x - (WORLD_SIZE / 2) as i64 * 32;
        let z_offset = at.z - (WORLD_SIZE / 2) as i64 * 32;
            ...
            (0..WORLD_SIZE).into_par_iter().for_each(|z| {
                (0..WORLD_SIZE).for_each(|y| {
                    ...
                    let f = |positon: UVec3| -> Option<u32> {
                        let chunk = &chunks[(positon.x / 16 + (positon.z / 16) * 2) as usize];
                        let chunk = chunk.as_ref();
                        let world_coords = WorldCoords {
                            x: (positon.x as i64 - x_offset) + chunk_pos.x as i64 * 32,
                            y: positon.y as i64 + 64 + chunk_pos.y as i64 * 32,
                            z: (positon.z as i64 - z_offset) + chunk_pos.z as i64 * 32,
                        };
        ...
        world.construct_octants_with(2, world_f);
        world
    }
}
