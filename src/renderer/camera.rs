...
impl Camera {
    ...
    pub fn look_at(eye: Vec3A, center: Vec3A, up: Vec3A, fov_radians: f32) -> Self {
    ...
    pub fn move_with_keyboard_input(&mut self, ctx: &Context) {
        ctx.input(|input| {
            ...
            if input.key_down(egui::Key::Space) {
                movement += Vec3A::Y * 0.1;
            }
            if input.modifiers.shift {
                movement += Vec3A::NEG_Y * 0.1;
            }
