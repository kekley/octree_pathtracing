...
impl<T: PartialEq + Default> Octree<T> {
                ...
                if child.is_leaf() && t_min > 0.0 {
                    ...
                    match model {
                        ...
                        ResourceModel::Quad(quad_model) => {
                            unmirrored_pos -= 1.0;
                            unmirrored_pos /= octree_scale;
                            let mut hit_point = (ro + rd * t_min);
                            hit_point -= 1.0;
                            hit_point /= octree_scale;
                            if quad_model.intersect(ray, &unmirrored_pos, &hit_point) {
                                return true;
                            } else {
                            }
                        }
