...
impl Octree<u32> {
    pub fn construct_parallel<P: Position, F: Fn(P) -> Option<u32> + Sync>(
        ...
        let a = (0u8..8)
            ...
            .map(|i: u8| {
                ...
                let mut subtree: Octree<u32> = Octree::with_capacity(5000);
            ...
            .collect::<Vec<Octree<u32>>>();
        let array: [Octree<u32>; 8] = a.try_into().unwrap();
    ...
    pub fn consume_octrees_of_depth(
        ...
        let child_array: [Child<u32>; 8] = array::from_fn(|_| Default::default());
        ...
        let mut new_octant_vec: Vec<Octant<u32>> = Vec::new();
        ...
        let mut new_octree: Octree<u32> = Octree {
            root: Some(0),
